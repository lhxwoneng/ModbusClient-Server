...
void CSocketClientDlg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHECK1, m_bRTUMode);
}
...
void CSocketClientDlg::OnDataReceived(CSocketHandle* pSH, const BYTE* pbData, DWORD dwCount, const SockAddrIn& addr)
{
    ...
    AppendText( _T("%s>(%s)\r\n"), strAddr, strText);
    std::vector<BYTE> data(dwCount);
    memcpy(data.data(), pbData, dwCount);
}
...
CString GetTokenLeftString(CString &strSrc, CString &strToken)
{
    if (strSrc.IsEmpty() || strToken.IsEmpty())
    {
        return _T("");
    }

    int nTokenPos = -1;
    nTokenPos = strSrc.FindOneOf(strToken);
    if (nTokenPos < 0)
    {
        // 没有找到，说明整个就是字符串
        return strSrc;
    ...
    {
        // 有分隔符说明有两个以上的字符串，这里是保存第一个字符串
        return strSrc.Left(nTokenPos);
    }

    return _T("");
}

void SplitStringToArray( CString strSrc, CString strToken, CStringArray &strArray )
{
    if (strSrc.IsEmpty() || strToken.IsEmpty())
    {
        return;
    ...
    strArray.RemoveAll();
    LPCTSTR lpszToken = strSrc;
    while (NULL != lpszToken)
    {
        // 获取token左边的字符串
        CString strElem = lpszToken;
        strElem = GetTokenLeftString(strElem, strToken);
        if (FALSE == strElem.IsEmpty())
        {
            strArray.Add(strElem);
        }

        // 获取余下的其他字符串
        lpszToken = FindOneOf(lpszToken, strToken);
    }
}
...
void CSocketClientDlg::OnBnClickedBtnSend()
{
    ...
    if ( m_SocketClient.IsOpen() )
    {
        ...
        if (m_bRTUMode)
        {
            ...
            SplitStringToArray(strMsg, _T(" "), strArray);
            if (strArray.IsEmpty())
            {
                MessageBox(_T("Please Enter the Hex String with seperator space!"));
                return;
            }

            for (int index = 0; index < strArray.GetSize(); index++)
            {
                BYTE ele = atoi(strArray[index]);
                data.push_back(ele);
            }
        }
        else
        {
            data.resize(strMsg.GetLength());
            memcpy(data.data(), (LPCTSTR)strMsg, strMsg.GetLength());
        }

        if (m_nSockType == SOCK_TCP)
        {
            m_SocketClient.Write(data.data(), data.size(), NULL);
        }
        else
        {
            SockAddrIn sockAddr;
            GetDestination(sockAddr);
            m_SocketClient.Write(data.data(), data.size(), sockAddr);
