...
bool SocketServerImpl<T, tBufferSize>::StartServer(LPCTSTR pszHost, LPCTSTR pszServiceName, int nFamily, int nType, UINT uOptions)
{
    ...
    if ( result )
    {
        _thread = (HANDLE)_beginthreadex(0, 0, (unsigned int (__stdcall *)(void *))SocketServerProc, this, 0, 0);
...
void SocketServerImpl<T, tBufferSize>::OnConnection(ULONG_PTR s)
{
    ...
    if ( _pInterface != NULL ) {
        _pInterface->OnThreadLoopEnter(&sockHandle);
    }
...
//     }
    ...
    while ( sockHandle.IsOpen() )
    {
        ...
        if ( ( dwRead != -1L ) && (dwRead > 0))
        {
            ...
            if ( _pInterface != NULL ) {
                _pInterface->OnDataReceived(&sockHandle, &data[0], dwRead, addrIn);
            }
        }
        else if (type == SOCK_STREAM && dwRead == 0L )
        {
            // connection broken
            if ( _pInterface != NULL ) {
                _pInterface->OnConnectionDropped(&sockHandle);
            }
            break;
        }
        else if ( dwRead == -1L)
        {
            ...
            if ( _pInterface != NULL )
            {
                ...
                _pInterface->OnConnectionError(&sockHandle, dwError);
            }
            else {
                break;
            }
    ...
    if ( _pInterface != NULL ) {
        _pInterface->OnThreadLoopLeave(&sockHandle);
    }
}
